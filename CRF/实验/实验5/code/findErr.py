        ...
        for j in echword:
            ...
            if len(ech)==3:
                if str(ech[1])!=str(ech[2]):
                    erosen=erosen+str(ech[0])+'error('+str(ech[1])+'|'+str(ech[2])+')'
                else:
                    erosen=erosen+str(ech[0])
        if 'error' in erosen:
...
def main():
    ...
    wrypath=r'F:\finderro'+systime+'.txt'

    #findErr(chaArry(read(txtpath)))
    #print(wry(chaArry(read(txtpath)),wrypath),'is ok!')
    print(wry(findErr(putall(chaArry(read(txtpath)))),wrypath),'is ok!')
